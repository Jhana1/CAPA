...
- (void)foo __attribute__((annotate("CAPA:enforce[prohibited call]")));
@end
...
TEST(ObjCVerifyProhibitedCallRule, ProtectedPropertyGetterOutside)
{
    ...
    @property (strong) A* a
        __attribute__((annotate("CAPA:enforce[prohibited call]")));
    @end
    @interface D : NSObject
    @property (strong) C* c;
    @end
...
TEST(ObjCVerifyProhibitedCallRule, ProhibitedFunctionCall)
{
    ...
    void foo(int x) __attribute__((annotate("CAPA:enforce[prohibited call]")));
...
{
    const string testFunction = R"END(
    void foo(int x) __attribute__((annotate("CAPA:enforce[prohibited call][bar]")));
    int main(int argc, char* argv[]) {
        foo(3);
        return 0;
    }
...
TEST(ObjCVerifyProhibitedCallRule, CommentMethod)
{
    ...
    - (void)foo __attribute__((annotate("CAPA:enforce[prohibited call][bar]")));

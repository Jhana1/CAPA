...
public:
    ...
    {
        /*node->dumpColor();
        std::cout << node->child_begin()->getStmtClassName();
        for (auto n = node->child_begin(); n != node->child_end(); ++n)
        {
            std::cout << std::endl;
            n->dumpColor();
        }*/
    ...
    bool VisitBinaryOperator(BinaryOperator *node)
    {
        /*for (auto n = node->child_begin(); n != node->child_end(); ++n) 
        {
            std::cout << n->getStmtClassName() << std::endl;
        }*/
        return true;
    }
     
    bool WalkUpFromBinaryOperator(BinaryOperator *node)
    {
...
static RuleSet rules(new Test1Rule());
